select country, campaign as campaign_id, count(distinct(event.identity_adid)) as active_user, regexp_replace(cast(sum(price) as VARCHAR), '(\d)(?=(\d\d\d)+(?!\d))', '$1,') as total_price
